...
public:
    MemoryBus(Cartridge& cartridge, Ppu& ppu, InterruptController& interrupts, Timer& timer,
              SerialPort& serial, Joypad& joypad, Apu& apu, bool cgb_mode,
              SpeedController& speed, InfraredPort& infrared)
        : cartridge_(cartridge), ppu_(ppu), interrupts_(interrupts), timer_(timer), serial_(serial),
          joypad_(joypad), apu_(apu), cgb_mode_(cgb_mode), speed_(speed), infrared_(infrared) {}
    ...
    /**
     * OAM DMA est cadencé sur le CPU : 1 octet / 4 cycles CPU. HDMA/GDMA est
     * cadencé sur l'horloge LCD : 1 octet / 2 dots. Ainsi le DMA OAM accélère
     * en double vitesse tandis que le nouveau DMA CGB garde la même durée
     * physique.
     */
    ...
    void load(BinaryReader& in) {
        in.raw(wram);
        in.raw(hram);
        if (in.i32() != 13) throw SaveStateError("État instantané corrompu (DMA)");
        dma_register_ = in.i32();
        oam_dma_active_ = in.i32() != 0;
        oam_dma_source_ = in.i32();
        oam_dma_index_ = std::clamp(in.i32(), 0, 0xa0);
        oam_dma_cycle_accum_ = std::max(0, in.i32());
        svbk_ = in.i32();
        hdma_source_ = in.i32();
        hdma_destination_ = in.i32();
        hdma_length_ = std::max(0, in.i32());
        hdma_hblank_ = in.i32() != 0;
        hdma_active_ = in.i32() != 0;
        hdma_block_bytes_remaining_ = std::clamp(in.i32(), 0, 16);
        hdma_dot_accum_ = std::max(0, in.i32());
        infrared_.load(in);
    }
...
private:
    ...
    [[nodiscard]] int read_internal(int address) {
        ...
        case 0xff4d: return cgb_mode_ ? speed_.read_key1() : 0xff;
        case 0xff4f: return ppu_.read_vram_bank();
        case 0xff51: return cgb_mode_ ? (hdma_source_ >> 8) & 0xff : 0xff;
        case 0xff52: return cgb_mode_ ? hdma_source_ & 0xf0 : 0xff;
        case 0xff53: return cgb_mode_ ? (hdma_destination_ >> 8) & 0x1f : 0xff;
        case 0xff54: return cgb_mode_ ? hdma_destination_ & 0xf0 : 0xff;
        case 0xff55: return read_hdma_status();
        case 0xff56: return infrared_.read();
        case 0xff68: return cgb_mode_ ? ppu_.read_bcps() : 0xff;
        case 0xff69: return cgb_mode_ ? ppu_.read_bcpd() : 0xff;
        case 0xff6a: return cgb_mode_ ? ppu_.read_ocps() : 0xff;
        case 0xff6b: return cgb_mode_ ? ppu_.read_ocpd() : 0xff;
        case 0xff70: return cgb_mode_ ? svbk_ | 0xf8 : 0xff;
    ...
    void write_internal(int address, int value) {
        ...
        case 0xff40: ppu_.write_lcdc(value); break;
        ...
        case 0xff4d: if (cgb_mode_) speed_.write_key1(value); break;
        case 0xff4f: ppu_.write_vram_bank(value); break;
        case 0xff51: if (cgb_mode_) hdma_source_ = (hdma_source_ & 0x00f0) | (value << 8); break;
        case 0xff52: if (cgb_mode_) hdma_source_ = (hdma_source_ & 0xff00) | (value & 0xf0); break;
        case 0xff53: if (cgb_mode_) hdma_destination_ = (hdma_destination_ & 0x00f0) | ((value & 0x1f) << 8); break;
        case 0xff54: if (cgb_mode_) hdma_destination_ = (hdma_destination_ & 0x1f00) | (value & 0xf0); break;
        case 0xff55: if (cgb_mode_) start_hdma(value); break;
        case 0xff56: infrared_.write(value); break;
        case 0xff68: if (cgb_mode_) ppu_.write_bcps(value); break;
        case 0xff69: if (cgb_mode_) ppu_.write_bcpd(value); break;
        case 0xff6a: if (cgb_mode_) ppu_.write_ocps(value); break;
        case 0xff6b: if (cgb_mode_) ppu_.write_ocpd(value); break;
        case 0xff70: if (cgb_mode_) svbk_ = value & 7; break;
    ...
    void start_dma(int high) noexcept {
        ...
        oam_dma_source_ = dma_register_ << 8;
        oam_dma_index_ = 0;
    ...
    void transfer_hdma_byte() noexcept {
        ...
        hdma_destination_ = (hdma_destination_ + 1) & 0x1fff;
    }
